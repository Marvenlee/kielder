...
/* #include "sgtty.h"       - Doesn't seem to be needed */
#include <stdlib.h>
#include <stdio.h>
...
static char termcap[1024];		/* termcap buffer */
static char tc[100];			/* area to hold string capabilities */
static char *ttytype;			/* terminal type from env */
static char *arp;			/* pointer for use in tgetstr */
static char *cp;			/* character pointer */

static char *cl;			/* clear screen capability */
static char *cm;			/* cursor motion capability */
static char *so;			/* start standout capability */
static char *se;			/* end standout capability */

static char nscrn[ROWS][COLS], cscrn[ROWS][COLS], mode;
static int row, col;
